...
class TestCursorPageHelperMethods(TestCase):
    """Test the before() and after() helper methods added to CursorPage"""
    ...
    def test_before_method_returns_cursor_for_first_item(self):
        page = self.paginator.page(first=3)
        before_cursor = page.before()
        expected_cursor = self.paginator.cursor(page[0])
        self.assertEqual(before_cursor, expected_cursor)

    def test_after_method_returns_cursor_for_last_item(self):
        page = self.paginator.page(first=3)
        after_cursor = page.after()
        expected_cursor = self.paginator.cursor(page[-1])
        self.assertEqual(after_cursor, expected_cursor)

    def test_before_method_with_empty_page(self):
        paginator = CursorPaginator(Post.objects.none(), ("-created",))
        page = paginator.page(first=10)
        self.assertIsNone(page.before())

    def test_after_method_with_empty_page(self):
        paginator = CursorPaginator(Post.objects.none(), ("-created",))
        page = paginator.page(first=10)
        self.assertIsNone(page.after())

    def test_before_after_cursors_for_navigation(self):
        """Test that before/after cursors can be used for pagination navigation"""
        # Get first page
        page1 = self.paginator.page(first=3)
        self.assertEqual(len(page1), 3)

        # Use after() cursor to get next page - should work without errors
        after_cursor = page1.after()
        self.assertIsNotNone(after_cursor)
        page2 = self.paginator.page(first=3, after=after_cursor)

        # Use before() cursor from second page - should work without errors
        before_cursor = page2.before()
        self.assertIsNotNone(before_cursor)
        page_back = self.paginator.page(last=3, before=before_cursor)

        # Basic validation that navigation works
        self.assertGreater(len(page_back), 0)

    def test_single_item_page_cursors(self):
        """Test before/after cursors work correctly with single-item pages"""
        page = self.paginator.page(first=1)
        before_cursor = page.before()
        after_cursor = page.after()

        # Both should point to the same item
        self.assertEqual(before_cursor, after_cursor)
        self.assertEqual(before_cursor, self.paginator.cursor(page[0]))

    def test_backward_pagination_with_helper_methods(self):
        """Test using helper methods with backward pagination (last/before)"""
        # Get last 3 items
        page = self.paginator.page(last=3)
        self.assertEqual(len(page), 3)
        self.assertSequenceEqual(page, [self.items[7], self.items[8], self.items[9]])

        # Use before() to get previous items
        prev_page = self.paginator.page(last=2, before=page.before())
        self.assertEqual(len(prev_page), 2)
        self.assertSequenceEqual(prev_page, [self.items[5], self.items[6]])
